...
class FewRel_RelationPrompt(object):
    def __init__(self, process):
        random.seed(34)
        self.data_path = '/raid/liuxiao/nell_data/fewrel/by_relation_generated'
        self.filenames = sorted(json.load(open(os.path.join(self.data_path, '../val_relations.json'))))
    ...
    def run(self):
        for filename in tqdm(self.filenames):
            ...
            relation = filename.replace('.json', '')
            print(relation)

            # get prompts to rank
            prompts = []
            for prompt in json.load(open(os.path.join(self.data_path, '..', 'freq_ranked_prompts', filename))):
                if re.search(r'[a-z]', prompt[0]) and prompt[1] > 1:
                    prompts.append(prompt)
            prompts = prompts[:20]

            data = json.load(open(os.path.join(self.data_path, filename)))
            data = random.sample(data, 100)
            for prompt in prompts:
                template = f"Sentence: [TEXT] [X] {prompt[0].strip(' ')} [MASK]"
                correct_times = 0
                for doc in data:
                    inputs = template.replace('[TEXT]', doc['text']).replace('[X]', doc['head'])
                    for iteration in range(3):
                        txts = self.process(inputs)
                        pred = extract_answer(inputs, txts[0])
                        if pred == doc['tail']:
                            correct_times += 1
                            break
                prompt.append(round(correct_times / 100, 4))
                # print(correct_times)
            prompts = sorted(prompts, key=lambda x: x[2], reverse=True)
            json.dump(prompts, open(f'/raid/liuxiao/nell_data/fewrel/ranked_prompts/{filename}', 'w'),
...
def main(args):
    ...
    fewrel_processer = FewRel_RelationPrompt(process)
    fewrel_processer.run()
