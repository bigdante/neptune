...
class FewRel_RelationPrompt(object):
    def __init__(self, process):
        self.data_path = '/raid/liuxiao/nell_data/fewrel/by_relation'
        self.filenames = sorted(list(os.listdir(os.path.join(self.data_path, 'val_relations.json'))))
    ...
    def run(self):
        for filename in self.filenames:
            ...
            for doc in tqdm(data):
                doc['generated_queries'] = []
                # for prompt in doc['prompts']:
                prompt = doc['prompts'][0]
                print(doc['id'], '\t', prompt)
                for iteration in range(2):
                    txts = self.process(prompt)
                    pred = extract_answer(prompt, txts[0])
                    print(pred)
                    doc['generated_queries'].append(pred)
            json.dump(data, open(f'/raid/liuxiao/nell_data/fewrel/by_relation_generated/{filename}', 'w'),
