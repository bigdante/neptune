...
class ConstrainedBeamSearchStrategy:
    def __init__(self, num_beams, length_penalty=1., consider_end=False,
                end_tokens=[], invalid_slices=[], no_repeat_ngram_size=0, min_tgt_length=0, tokenizer=None):
        ...
        self.context_tokens, self.indices, self.mask = None, None, None

    def refresh(self):
        self.context_tokens, self.indices, self.mask = None, None, None

    def create_mask(self, batch_size, vocab_size, indices, device):
        mask = torch.full((batch_size, vocab_size), -1e9).float().to(device)
        for idx in range(batch_size):
            mask[idx, indices[idx]] = 0
        self.mask = mask

    def init_constraints(self, tokens):
        self.context_tokens = tokens
        self.indices = []
        for batch_idx in range(tokens.shape[0]):
            self.indices.append(torch.LongTensor(
                sorted(list(set(tokens.detach().cpu().tolist()[batch_idx])) +
                       self.command_tokens)
            ).to(tokens.device))
