...
def filling_sequence_calibrate(
        model,
        seq,
        counter_seq,
        batch_size,
    ...
    # building the initial tokens, attention_mask, and position_ids
    context_length, _context_length = 0, 0
    while seq[context_length] >= 0:
        context_length += 1 # [0, context_length-1] are given
    while counter_seq[_context_length] >= 0:
        _context_length += 1
    assert context_length > 0
    tokens, attention_mask, position_ids = get_masks_and_position_ids(seq)
    tokens = tokens[..., :context_length]
    ...
    # TODO calibrate
    _tokens, _attention_mask, _position_ids = get_masks_and_position_ids(counter_seq)
    _tokens = _tokens[..., :_context_length]
    _attention_mask = _attention_mask.type_as(next(model.parameters()))

    _counter = _context_length - 1 # Last fixed index is ``counter''
    _index = 0 if mems is None else mems.shape[2] # Next forward starting index, also the length of cache.

    while _counter < len(counter_seq) - 1:
        # Now, we want to generate seq[counter + 1],
        # token[:, index: counter+1] needs forwarding.

        if counter_seq[_counter + 1] >= 0: # provided
            tokens = torch.cat(
                (
                    _tokens,
                    counter_seq[_counter+1: _counter+2].expand(_tokens.shape[0], 1)
                ), dim=1
            )
            _counter += 1
            continue

        _logits, *mem_kv = model(
            _tokens[:, _index:],
            _position_ids[..., _index: _counter+1],
            _attention_mask[..., _index: _counter+1, :_counter+1], # TODO memlen
            mems=None,
            log_attention_weights=None
        )
        _logits = _logits[:, -1].expand(batch_size, -1) # [batch size, vocab size]
        _tokens = _tokens.expand(batch_size, -1)
        strategy.get_calibrated_weight(_logits, _tokens, None)
        break
    ...
    while counter < len(seq) - 1:
        ...
        logits = logits[:, -1].expand(batch_size, -1) # [batch size, vocab size]
        tokens = tokens.expand(batch_size, -1)
        tokens, mems = strategy.forward(logits, tokens, mems, is_first_token=first_token)
        first_token = False
        if strategy.is_done:
            break
    return strategy.finalize(tokens, mems)
