...
class BlockDataset(data.Dataset):
    ...
    def get_weighted_samples(self, np_rng):
        while True:
            ...
            tokens, loss_mask, ssm_idx = self.getidx(idx)
            if self.filter_english:
                ...
                break
        return tokens, loss_mask, ssm_idx
    ...
    def __getitem__(self, idx):
        ...
        tokens, loss_mask, ssm_idx = self.get_weighted_samples(rng)
        # print_rank_0("## SSM_IDX: " + str(ssm_idx))
        ...
        if tokens_to_strip > 0:
            ...
            ssm_spans = [[ssm[0] + 1, ssm[1] + 1] for ssm in ssm_idx if ssm[1] + 1 <= len(tokens) - 1]
        else:
            tokens = [self.tokenizer.get_command('ENC').Id] + tokens
            loss_mask = [0] + loss_mask
            ssm_spans = [[ssm[0] + 1, ssm[1] + 1] for ssm in ssm_idx]
        return {'text': np.array(tokens), "loss_mask": np.array(loss_mask), "ssm_idx": ssm_spans}
    ...
    def getidx(self, data_idx):
        ...
        assert len(data['ssm_idx']) % 2 == 0
        ssm_idx = [tuple([data['ssm_idx'][2 * i], data['ssm_idx'][2 * i + 1]]) for i in range(len(data['ssm_idx']) // 2)]
        _ssm_idx = []
        for s in ssm_idx:
            if s[0] < s[1]:
                _ssm_idx.append(s)
        if len(_ssm_idx) == 0:
            raise RuntimeError("No ssm data. {} contains wrong data".format(ssm_idx))
        ssm_idx = _ssm_idx
        return tokens, loss_masks, ssm_idx
