...
def get_dataset(name, tokenizer, pre_tokenize, data_parallel_rank, loader_scatter=None, no_lazy_loader=False,
    ...
    if not (exists_lazy(path, data_type='text')) and not (
        ...
        if global_rank == 0:
            ...
            writer = MultiLazyWriter(path, data_types=['prompt', 'text', 'ssm_idx'], is_array=pre_tokenize)
            reader = dataset(writer=writer, tokenizer=tokenizer, tokenize=pre_tokenize)
            reader.process()
            writer.close()
        else:
            while not os.path.exists(LazyWriter.get_len_path(path, data_type='prompt')) or not os.path.exists(
                    LazyWriter.get_len_path(path, data_type='text')) or not os.path.exists(
                LazyWriter.get_len_path(path, data_type='ssm_idx')):
                time.sleep(1)
    ...
    texts = LazyLoader(lazy_path, data_type='text', map_fn=map_fn, mem_map=True,
                       is_array=pre_tokenize, load_memory=no_lazy_loader, loader_fraction=loader_fraction)
    text = corpora.PromptSSMDataset(prompt_loader=prompts, text_loader=texts, ssm_loader=ssm_indices,
                                    tokenizer=tokenizer,
