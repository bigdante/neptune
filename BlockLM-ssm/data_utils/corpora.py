...
class PromptSSMDataset(data.Dataset):
    def __init__(self, prompt_loader, text_loader, ssm_loader, tokenizer=None, to_tokenize=False, name="", **kwargs):
        self._name = name
        self.prompts = prompt_loader
        self.texts = text_loader
        self.ssm = ssm_loader
        self._tokenizer = tokenizer
        self.to_tokenize = to_tokenize
        if (self.prompts is None or isinstance(self.prompts, LazyLoader)) and isinstance(self.texts,
                                                                                         LazyLoader) and isinstance(
                self.ssm, LazyLoader):
            self.prompt_lens = self.prompts.lens if self.prompts else None
            self.text_lens = self.texts.lens
            self.ssm_lens = self.ssm.lens
            self.is_lazy = True
    ...
    def __getitem__(self, index):
        prompt = self.prompts[index] if self.prompts else []
        text = self.texts[index]
        ssm_idx = self.ssm[index]
        if self.to_tokenize:
            prompt = self.tokenizer.EncodeAsIds(prompt).tokenization if prompt else []
            text = self.tokenizer.EncodeAsIds(text).tokenization
        return {"tokens": prompt + text, "loss_masks": [0] * len(prompt) + [1] * len(text), 'ssm_idx': ssm_idx}
    ...
    def process(self):
        ...
        while True:
            ...
            if data == 'COMPLETE':
                ...
                #except:
                #    print("################### Error when write to lazy loader ########################")
                #    print(data)
                #    exit(1)
                progress_bar.update()
        progress_bar.close()
        ...
        for path in self.get_paths():
            ...
            with open(path) as file:
                for i, row in enumerate(file):
                    if torch.distributed.get_rank() == 0 and i % 1000 == 0:
                        tqdm_print(f"Read in {i} lines...")
                    task_queue.put(row)
...
class WikipediaSSM(LineReader):
    """
    Salient Span Masking Format
    text: string
    entities: list of
    """
    PATH = "/dataset/fd5061f6/english_data/wikipedia_ssm/en_wiki_ssm_sampled_docs_cleaned.jsonl"
    reserve_punct = True
    # TODO: 这里写的ssm_span有问题，应该是写错了！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    def process_line(self, data, tokenizer, tokenize):
        prompt, tokens = "", []
        ssm_span = []
        prompt = self.process_sample(prompt, tokenizer, tokenize)
        ptr, all_text = 0, ''
        for ssm_idx in data['ssm_idx']:
            if ssm_idx > ptr:
                prefix_text = ''.join(data['texts'][ptr:ssm_idx])
                tokens.extend(self.process_sample(prefix_text, tokenizer, tokenize))
                all_text = all_text + prefix_text
            ssm_text = ('' if all_text == '' or all_text[-1] != ' ' else ' ') + ''.join(data['texts'][ssm_idx])
            ssm_tokens = self.process_sample(ssm_text, tokenizer, tokenize)
            ssm_span.extend([len(tokens), len(tokens) + len(ssm_tokens)])
            # postprocess
            tokens.extend(ssm_tokens)
            ptr = ssm_idx + 1
            all_text = all_text + data['texts'][ssm_idx]
        if ptr < len(data['texts']):
            tokens.extend(self.process_sample(''.join(data['texts'][ptr:]), tokenizer, tokenize))
        return [{"prompt": prompt, 'text': tokens, 'ssm_idx': ssm_span}]
