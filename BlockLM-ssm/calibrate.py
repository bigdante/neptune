...
from SwissArmyTransformer.generation.autoregressive_sampling import filling_sequence_calibrate
from SwissArmyTransformer.generation.sampling_strategies import BeamSearchStrategy, BaseStrategy, \
    ConstrainedBeamSearchStrategy, ConstrainedBaseStrategyCalibrate
...
class Server(object):
    ...
    def predict(self, queries):
        ...
        for query in queries:
            txts = self.process(query)
            pred = extract_answer(query, txts[0])
            answers.append(pred)
        return answers
...
def main(args):
    ...
    if args.num_beams == 1:
        if args.inference_strategy_constrained:
            strategy = ConstrainedBaseStrategyCalibrate(temperature=args.temperature, top_k=args.top_k,
    ...
    def process(raw_text):
        ...
        seq = tokenizer.EncodeAsIds(raw_text.replace('<head> ', '').replace('</head> ', '')).tokenization
        counterfact_seq = tokenizer.EncodeAsIds(re.sub(r'<head>.*?</head>', 'N/A', raw_text)).tokenization
        context_seq = tokenizer.EncodeAsIds(raw_text.split('</s></s>')[1]).tokenization
        strategy.context_tokens = torch.cuda.LongTensor([context_seq], device=args.device)
        seq = [tokenizer.get_command('ENC').Id] + seq
        counterfact_seq = [tokenizer.get_command('ENC').Id] + counterfact_seq
        ...
        while True:
            ...
            for tim in range(max(args.batch_size // mbz, 1)):
                input_seq = torch.cuda.LongTensor(
                    ...
                    device=args.device)
                counterfact_input_seq = torch.cuda.LongTensor(
                    counterfact_seq + [tokenizer.get_command('sop').Id] + [-1] * (args.out_seq_length - len(seq) - 1),
                    device=args.device
                )
                output = filling_sequence_calibrate(model, input_seq, counterfact_input_seq,
    ...
    #     return output_process(processor.predict(queries))
    #
    # app.run(host="0.0.0.0", port=21540)
