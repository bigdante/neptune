...
class ConstructBlockStrategy:
    ...
    def generate_blank_data(self, sample, masked_lengths, rng, task='bert'):
        ...
        # TODO: substitute sampled spans with ssm spans
        #block_spans = self.sample_span_in_document(tokens, masked_lengths, rng)
        block_spans = sample['ssm_idx']
        ssm_text = []
        for s in block_spans:
            ssm_text.append(''.join(self.tokenizer.DecodeIds(tokens[s[0]: s[1]])))
        # print_rank_0("### block_spans: " + str(ssm_text) + str(block_spans))
        ...
        if rand < self.bert_prob:
            ...
            for sample in samples:
                ...
                multiple_doc = index_in_list(sample['text'], self.tokenizer.get_command('eos').Id) not in [-1, len(
                    sample['text']) - 1]
                if False and (multiple_doc or rng.random() < self.infill_prob):
