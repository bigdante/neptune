...
class ControllerV1_1:
    def __init__(self, process_id=0):
        ...
        # self.true_offset = self.process_id * 218323
        self.id_list = list(line.strip('\n') for line in open(f'/raid/liuxiao/nell_data/ids/{self.process_id}.json'))
    ...
    def yield_sentence(self):
        for sentence_id in self.id_list:
            sentence = BaseSentence.objects.get(id=sentence_id)
            yield sentence

    def get_sentence(self):
        for idx, sentence in tqdm(enumerate(self.yield_sentence())):
            self.sentence_offset += 1
...
if __name__ == '__main__':
    print(sys.argv)
    controller = ControllerV1_1(process_id=int(sys.argv[1]))
    controller.run()
